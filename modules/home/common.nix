{
  ...
  cfg = config.apps;
  mkProgramOption = name: lib.mkOption {
    type = lib.types.bool;
    default = true;
...
in {
  ...
  # TODO ==> Clean up, probably remove the redundant seperation
  ...
  options.apps = {
    ...
    niri-utils.enable = mkProgramOption "Niri Utilities (brightnessctl, playerctl, grim, etc.)";
  };
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.niri-utils.enable {
      home-manager.users.${globals.userName}.home.packages = with pkgs; [
        wl-clipboard
        brightnessctl
        playerctl
