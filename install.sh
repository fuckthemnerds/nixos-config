...
step "Bootstrapping Administrator Age Key..."
# Set the path where the admin key will be temporarily stored
ADMIN_KEY_FILE="/tmp/admin_key.txt"

if [[ ! -f "$ADMIN_KEY_FILE" ]]; then
    echo "No admin key found at $ADMIN_KEY_FILE. Generating a new one..."
    # Generate the key pair silently
    nix "${NIX_OPTS[@]}" shell nixpkgs#age --command age-keygen -o "$ADMIN_KEY_FILE" > /dev/null 2>&1
    ok "New master administrator key generated."
else
    ok "Using existing master administrator key from $ADMIN_KEY_FILE."
fi

# Extract the public key string from the file
...
if [[ ! -f /mnt/persistent/etc/nixos/secrets/secrets.yaml ]]; then
    ...
    # Prompt for secret Git credentials
	...
	ok "Existing secrets.yaml found. Skipping password and git credential hashing."
fi
...
nix "${NIX_OPTS[@]}" \
	...
	if [[ -f /mnt/persistent/etc/nixos/secrets/secrets.yaml ]]; then
		...
		echo "1. On your existing machine with the admin key, add this key to .sops.yaml"
		echo "2. Run 'sops updatekeys secrets/secrets.yaml' on the existing machine"
        ...
        # Pass both public keys directly to the encryption command
	...
	unset HASHED_PASSWORD GIT_USER GIT_EMAIL
EOF
